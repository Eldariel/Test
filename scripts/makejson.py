...
type -- The parent model for items. Default is "generated" an should not be changed in most cases.
...
def writeItemJSON(name, texture, layer=0, item_type='generated'):
	...
	"""
	print('Writting item %s (texture %s)' % (name, texture))
...
for arg in sys.argv:
	...
	matchCount = re.compile('count=').match(arg1)
	matchLayer = re.compile('layer=').match(arg1)
	matchTexture = re.compile('texture=').match(arg1)
	matchType = re.compile('type=').match(arg1)
	...
	if matchNum or matchCount: # subtype count
		...
		layer = int(re.search('\d+', arg1).group(0))
